...
export function gatRecordList(pageSize = 20, pageNum = 1, data) {
  var url = `/bank/recharge/bankRechargeList?pageSize=${pageSize}&pageNum=${pageNum}`
  ...
  if (data.orderName != null && data.orderName != undefined && `${data.orderName}` != 'null' && `${data.orderName}` != '') {
    url += `&orderName=${data.orderName}`
  }
  var startTime = null;
  var endTime = null;
...
export function gatTotalList(pageSize = 20, pageNum = 1, data) {
  var url = `/bank/recharge/total?pageSize=${pageSize}&pageNum=${pageNum}`
  if (data.bankId != null && data.bankId != undefined && `${data.bankId}` != 'null' && `${data.bankId}` != '') {
    url += `&bankId=${data.bankId}`
  }
  if (data.uid != null && data.uid != undefined && `${data.uid}` != 'null' && `${data.uid}` != '') {
...
export function checkStatus(id, status) {
  return request({
    url: `/bank/recharge/checkRecharge?id=${id}&status=${status}`,
    method: 'get'
  })
}
...
export function checkErrorStatus(id, status) {
  return request({
    url: `/bank/recharge/checkErrorRecharge?id=${id}&status=${status}`,
    method: 'get'
  })
}
export function getBankList() {
  return request({
    url: `/bank/recharge/getBankList?type=7`,
    method: 'get'
  })
}
export function getupdate_Time() {
  return request({
    url: `/bank/recharge/getupdate_Time`,
    method: 'get'
  })
}
export function update_Time(num) {
  return request({
    url: `/bank/recharge/update_Time?&num=${num}`,
    method: 'get'
  })
}
