...
export function getTransList(pageSize = 20, pageNum = 1, data) {
  var url = `/fb/trans?pageSize=${pageSize}&pageNum=${pageNum}`
  ...
  if (data.seriesType != null && data.seriesType != undefined && `${data.seriesType}` != 'null' && `${data.seriesType}` != '') {
    url += `&seriesType=${data.seriesType}`
  }
  if (data.orderStatus != null && data.orderStatus != undefined && `${data.orderStatus}` != 'null' && `${data.orderStatus}` != '') {
    url += `&orderStatus=${data.orderStatus}`
  }
  if (data.orderNum != null && data.orderNum != undefined && `${data.orderNum}` != 'null' && `${data.orderNum}` != '') {
    url += `&orderNum=${data.orderNum}`
  }
  var startTime = null;
  var endTime = null;
...
export function getTransTotal(pageSize = 20, pageNum = 1, data) {
  var url = `/fw/trans/total?pageSize=${pageSize}&pageNum=${pageNum}`
  if (data.type != null && data.type != undefined && `${data.type}` != 'null' && `${data.type}` != '') {
    url += `&type=${data.type}`
  }
  if (data.uid != null && data.uid != undefined && `${data.uid}` != 'null' && `${data.uid}` != '') {
...
export function getDetail(orderNum) {
  return request({
    url: '/fb/trans/getDetail?orderNum=' + orderNum,
    method: 'get'
  })
}
