 ...
 * @param {Object} file
 */
export function uploadFileadver(ossParams, path, fileName, file) {
  return new Promise((resolve, reject) => {
    ...
    var uri = nullS(path);
    uri = uri.replace(/\//g, '_');
    const file_name = 'adver' + '_' + Date.parse(new Date()) + suffix; // 组成新文件名
    ...
    client
      ...
      .catch(err => {
        console.log(11111111, JSON.stringify(err))
        reject(err);
      });
  })
}
