<template>
  <div class="app-container">
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini" fit highlight-current-row show-summary
      ...
      <el-table-column prop="oneTotalOrderNumToA" label="单笔充值订单数(100以内)" min-width="170" />
      <el-table-column prop="totalAmtToA" label="充值总额(100以内)"  min-width="150" />
      <el-table-column prop="oneTotalOrderNumToB" label="单笔充值订单数(100-1w)" min-width="180"  />
      <el-table-column prop="totalAmtToB" label="充值总额(100-1w)" min-width="180"  />
      <el-table-column prop="oneTotalOrderNum" label="单笔充值订单数(1w以上)" min-width="180"  />
      <el-table-column prop="totalAmtToC" label="充值总额(1w以上)" min-width="180"  />
  ...
  <script>
import { paymentStatis, paymentStatisTotal } from '@/api/report/paymentStatis'
import mixin from '../mixin'
import { getDateTime } from '@/utils/tools'
// const {NOW_TIME_START, NOW_TIME_END} = getDateTime()

export default {
  name: 'ReportPaymentStatisIndex',
  data() {
    return {
      list: [],
      listLoading: true,
