<template>
  <div class="app-container">
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini" fit highlight-current-row show-summary
      ...
      <el-table-column prop="payType" label="代付类型" />
      <el-table-column prop="orderNum" label="订单笔数" />
      <el-table-column prop="validOrderNum" label="有效订单笔数" />
      <el-table-column prop="failOrderNumUser" label="订单失败笔数" />
      <el-table-column prop="payAmt" label="代付金额" />
    </el-table>
  ...
  <script>
import { payStatis, payStatisTotal } from '@/api/chreport/payStatis'
import mixin from '../mixin'
import { getDateTime } from '@/utils/tools'
// const {NOW_TIME_START, NOW_TIME_END} = getDateTime()

export default {
  name: 'ChReportPayStatisIndex',
  data() {
    return {
      list: [],
      listLoading: true,
  ...
  methods: {
    ...
    async fetchData() {
      ...
      // 请求服务器
      try {
        const res = await payStatis(params)
      ...
      this.totalList = []
      try {
        const res = await payStatisTotal(params)
