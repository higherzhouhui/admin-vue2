...
<script>
import { getGameRankList } from "@/api/game/gamebet";
import { emptyS, nullS, formatUnixTime } from "@/utils/tools";
import UserDrawer from "@/components/UserDrawer";

export default {
  name: "GameRankController",
  filters: {},
  components: {
    UserDrawer,
  },
  data() {
    return {
      ...
      // statusOptions: [{ value: 1, label: '日榜' }, { value: 2, label: '周榜' }, { value: 3, label: '月榜' }],
  ...
  methods: {
    ...
    fetchData() {
      ...
      getGameRankList(this.listQuery)
        .then((res) => {
          this.list = (res.data.list || []).map((item) => {
            ...
            this.$set(item, "sendTypeString", sendTypeString);
