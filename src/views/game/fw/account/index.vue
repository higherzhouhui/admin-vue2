        ...
        >
          <el-form-item label="用户id" prop="uid"
            ><el-input
              ...
              maxlength="4"
          /></el-form-item>
          <el-form-item label="游戏内用户id" prop="fwAccountId"
            ><el-input
              v-model="createModel.fwAccountId"
              clearable
              placeholder="请输入游戏内用户id"
              maxlength="20"
              show-word-limit
...
<script>
import { getAccountList, updateAccount, autoUp } from "@/api/game/fw/account";
import { emptyS, nullS, formatUnixTime } from "@/utils/tools";

export default {
  name: "FwGameAccountController",
  filters: {
    statusFilter(status) {
    ...
    handleDataClick() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          if (formDic.fwAccountId == null || formDic.fwAccountId < 0) {
          ...
          const params = {
            uid: formDic.uid,
            fwAccountId: formDic.fwAccountId,
