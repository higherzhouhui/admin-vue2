<template>
  <div class="app-container">
    ...
    <el-form :inline="true" class="searchFormWrapper">
      ...
      <el-form-item>
        <el-input v-model="listQuery.uid" size="small" style="width: 125PX" class="input-with-select"
      ...
      <el-form-item>
        <el-select v-model="listQuery.isIncrease" size="small" placeholder="请选择类型" clearable style="width: 120px"
          @change="handleSearchFilter">
          <el-option v-for="item in isIncreaseOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-select v-model="listQuery.type" filterable size="small" placeholder="请选择来源" style="width: 120px" clearable
          @change="handleSearchFilter">
          <el-option v-for="item in typeOptions" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-input v-model="listQuery.trn" size="small" style="width: 170px" class="input-with-select" placeholder="请输入订单号"
    ...
    <el-form :inline="true">
      <el-form-item>
        <span type="primary">总金币：{{ todayList.length > 0 ? todayList[0].totalGoldCoin.toLocaleString() : 0 }}</span>
      </el-form-item>
    </el-form>
    <el-table v-cloading="listLoading" :data="list" size="mini">
      ...
      <el-table-column prop="oldCoin" label="变更前金币" sortable />
      <el-table-column prop="goldCoin" label="变更金币" sortable />
      <el-table-column prop="endCoin" label="变更后金币" sortable />
      ...
      <el-table-column label="来源" width="200">
        <template slot-scope="{ row }">
          <el-tag size="medium" :type="row.isIncrease ? 'info' : 'success'">{{
            row.stringString
          }}</el-tag>
        </template>
      </el-table-column>
...
<script>
import { getAssetsPlusList, getAssetsPlus } from '@/api/center/assets/chplus'
...
export default {
  name: 'ChCenterUserAssetsPlusController',
  filters: {
    typeFilter(status) {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      // getAssetsPlus(this.listQuery.limit, this.listQuery.page, this.listQuery)
      //   .then((res) => {
      //     var totallist = []
      //     if (
      //       res.data != null &&
      ...
      getAssetsPlusList(
        ...
        if (
          res.data.ext != null &&
          res.data.ext != undefined &&
          JSON.stringify(res.data.ext) != '{}'
        ) {
          totallist.push(res.data.ext)
        }
        this.todayList = totallist
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['用户ID', '变更前金币', '变更金币', '变更后金币', '类型', '来源', '订单号', '订单创建时间']
        const filterVal = ['uid', 'oldCoin', 'goldCoin', 'endCoin', 'typeString', 'stringString', 'trn', 'gmtCreateString']
