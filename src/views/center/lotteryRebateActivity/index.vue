<template>
  <div class="app-container">
    ...
    <el-form :inline="true">
      <!-- <el-form-item> -->
      <!-- <el-tag type="primary">返水日期:</el-tag> -->
      <!-- <el-date-picker v-model="listQuery.pickerValue" size="small" clearable type="datetimerange" range-separator="~"
          start-placeholder="开始日期" end-placeholder="结束日期" style="width: 330px" align="right" format="yyyy-MM-dd"
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini" fit highlight-current-row>
      ...
      <el-table-column prop="waterDate" label="返水日期" min-width="100" />
      ...
      /> -->
      <el-table-column prop="waterAmt" label="彩票流水" min-width="100" sortable />

      <el-table-column prop="giveAmt" label="赠送金币" min-width="100" sortable />
      <el-table-column prop="bonusMultiple" label="红利倍数" min-width="100" sortable />
    ...
    <el-dialog :title="dialogTitleMap[dialogStatus]" :visible.sync="dialogCreateVisible" :close-on-click-modal="false"
      ...
      <el-form ref="dataForm" v-cloading="dialogCreateLoading" :model="createModel" label-position="top"
        label-width="150px" :rules="rules" size="medium" class="form">
        ...
        <el-form-item v-for="(item, index) in createModel.conditions" :key="index" :label="`日彩票流水${index + 1}：`">
          <div class="liushui">
            <el-input v-model="item.waterAmt" placeholder="请输入日彩票返水" />
            <div class="mtitle">
              <span>赠送金币：</span>
              <el-input v-model="item.giveAmt" placeholder="请输入赠送金币" />
            </div>
            <div>
        ...
        <el-form-item label="流水统计：" prop="">
          <el-checkbox-group v-model="createModel.types">
            <el-checkbox v-for="item in gameTypes" :key="item.code" :label="item.code">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
...
<script>
import {
  getLotteryBackwaterConfig,
  giveAllRecord,
  giveRecord,
  getLotteryList,
  updateLotteryBackwaterConfig,
} from "@/api/center/lotteryRebateActivity/index";
...
export default {
  ...
  data() {
    return {
      ...
      gameTypes: [
        ...
        { name: "时时彩", code: "txssc" },
        { name: "六合彩", code: "yflhc" },
        { name: "河内彩票", code: "yn_hncp" },
        { name: "越南30s", code: "yncp30s" },
        { name: "幸运飞艇", code: "xyft" },
        { name: "彩蝶", code: "ft" },
        { name: "骰宝", code: "tz" },
        { name: "1分赛车", code: "race1m" },
        { name: "5分快三", code: "jsks5" },
      ],
      pickerOptions: {
        shortcuts: [
          {
  ...
  methods: {
    ...
    showCreateDialogClick() {
      ...
      getLotteryBackwaterConfig().then((res) => {
        if (res.data) {
          this.createModel = {
            ...res.data,
    ...
    validateValues() {
      ...
      if (emptyFlag) {
        this.$message({
          type: "warning",
          message: "日彩票流水/赠送金币不可为空",
        });
        return;
      }
      if (!this.createModel.types.length) {
        this.$message({
          type: "warning",
          message: "请选择流水统计",
        });
