<template>
  <div class="app-container">
    <el-form :inline="true">
      <el-form-item>
<!--        <el-button size="small" type="success" icon="el-icon-plus" @click="showCreateDialogClick()">新增</el-button>-->
      </el-form-item>
      <!-- 状态 -->
<!--      <el-form-item>-->
...
<!--      </el-form-item>-->
      ...
      <el-form-item>
        <el-button size="small" plain icon="el-icon-search" @click="handleSearchFilter" />
      </el-form-item>
    </el-form>
    ...
    <el-table v-cloading="listLoading" :data="list" highlight-current-row size="mini"  >
      ...
      <el-table-column prop="secretkey" label="谷歌秘钥" min-width="60"   />
...
<!--        <template slot-scope="{ row }">-->
<!--          <el-button plain type="primary" size="mini" :disabled="row.id == 1 || row.id == 2|| row.id==3" @click="showUpdateDialogClick(row)">编辑</el-button>-->
<!--        </template>-->
<!--      </el-table-column>-->
    </el-table>
    ...
    <el-dialog :title="dialogTitleMap[dialogStatus]" :visible.sync="dialogCreateVisible"   :close-on-click-modal="false" :destroy-on-close="true" width="35%">
      <div v-cloading="dialogCreateLoading" style="width: 100%;height: 100%;">
        <el-form ref="dataForm" :model="createModel" :rules="rules" label-position="top" size="medium">
          <el-form-item label="类型名称" prop="typeName"><el-input v-model="createModel.typeName" clearable placeholder="请输入类型名称" maxlength="200" /></el-form-item>
          <el-form-item label="排序" prop="level"><el-input v-model="createModel.level" clearable placeholder="请输入排序值" maxlength="200" /></el-form-item>
          <el-form-item label="0启用 1禁用">
            <el-switch v-model="createModel.status" active-text="启用" :active-value="0" inactive-text="禁用" :inactive-value="1" />
          </el-form-item>
        </el-form>
      </div>
      <div slot="footer">
...
<script>
import { getAnchorTypeList} from '@/api/center/user/google'
import { emptyS, nullS, formatUnixTime } from '@/utils/tools'

export default {
  name: 'CenterUserGoogleController',
  filters: {
    statusFilter(status) {
  ...
  data() {
    return {
      ...
      // 验证规则
      rules: {
        username: [{ required: true, message: '请输入类型名称', trigger: 'change' }, { min: 1, max: 200, message: '最多为200个字符', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getAnchorTypeList(this.listQuery.limit, this.listQuery.page, this.listQuery)
        .then(res => {
