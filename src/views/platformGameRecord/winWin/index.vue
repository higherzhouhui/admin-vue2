<template>
  <div class="app-container">
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini">
      ...
      <el-table-column prop="gameCode" label="游戏code" min-width="100" />
      <el-table-column prop="lottery" label="彩票code" min-width="100" />
      <el-table-column prop="amount" label="下注金额" min-width="100" />
      <el-table-column prop="betCount" label="投注数量" min-width="100" />
      <el-table-column prop="totalAmount" label="总投注金额" width="120" />
      <el-table-column prop="betOdds" label="赔率" width="100" />
      <el-table-column prop="betCode" label="投注code" min-width="100" />
      <el-table-column prop="bid" label="BetId" min-width="100" />
      <el-table-column prop="channel" label="渠道" min-width="100" />
      <el-table-column prop="cm" label="已结算投注" width="100" />
      <el-table-column prop="content" label="详情" width="100" />
      <el-table-column prop="created" label="下注时间" min-width="100" />
      <el-table-column prop="dividend" label="实际输赢" min-width="100" />

      <el-table-column prop="lotteryName" label="彩票名称" min-width="120" />
      <!-- <el-table-column prop="number" label="Deaw" width="100" /> -->

      <el-table-column prop="odds" label="最终结果" width="100">
        <!-- <template slot-scope="{row}">
{{  }}
      </template> -->
      </el-table-column>
      <!-- <el-table-column prop="realAmount" label="有效金额" width="100" /> -->
      <el-table-column prop="totalAmount" label="有效金额" width="100" />
      <!-- <el-table-column prop="result" label="result" width="100" /> -->
      <el-table-column prop="settled" label="是否已结算" width="100">
      <template slot-scope="{row}">
        {{
            row.settled == 1
              ? "已结算"
      ...
      <el-table-column prop="settledTime" label="结算时间" width="100" />
      ...
      <el-table-column prop="startTimeString" label="开始时间" width="100" />
      <el-table-column prop="endTimeString" label="结束时间" width="100" />
    </el-table>
    <!-- 分页 -->
    ...
    <!-- 游戏数据手动补偿 -->
    <el-dialog title="游戏数据补偿" :visible.sync="dialogCreateVisible" :close-on-click-modal="false"
      :destroy-on-close="true">
      <div v-cloading="dialogCreateLoading" style="width: 100%; height: 100%">
        <el-form ref="dataForm" :inline="true" :model="createModel" label-position="top" label-width="100px"
          size="medium" style="display: flex; flex-direction: column; align-items: center">
          <div style="
              display: flex;
...
<script>
import { recordXg } from '@/api/platformGameRecord/winWin'
...
export default {
  name: 'platformGameRecordXgRealPersonIndex',
  components: {
    UserDrawer,
  },
  data() {
      ...
      recordXg(this.listQuery.limit, this.listQuery.page, this.listQuery)
        .then((res) => {
          ...
          (res.data.list || []).map(item => {
            item.startTimeString = formatUnixTime(item.startTime)
            item.endTimeString = formatUnixTime(item.endTime)
          })
