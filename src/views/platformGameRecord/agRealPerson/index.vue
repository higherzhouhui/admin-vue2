  ...
  <div class="app-container">
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini">
      ...
      <el-table-column prop="type" label="类型" min-width="100">
        <template slot-scope="{ row }">
          {{ row.type == 1 ? "真人" : row.type == 2 ? "电子" : "" }}
        </template>
      </el-table-column>
      <el-table-column prop="billNo" label="订单号" min-width="100" />
      <el-table-column prop="currency" label="投注币种" min-width="100" />
      <el-table-column prop="gameType" label="游戏类型" width="100" />
      <el-table-column prop="betIp" label="投注IP" min-width="100" />
      <el-table-column prop="flag" label="订单状态" min-width="100">
        <template slot-scope="{ row }">
          {{
            row.flag == 0
              ? "异常(请联系客服)"
      ...
      <el-table-column prop="deviceType" label="设备" width="100">
        <teleport slot-scope="{ row }">
          {{ row.deviceType == 0 ? "pc" : row.deviceType == 1 ? "MOBILE" : "" }}
        </teleport>
      </el-table-column>
      <el-table-column prop="playName" label="用户名" min-width="100" />
      <el-table-column prop="gameCode" label="局号" min-width="100" />
      <el-table-column prop="netAmount" label="派彩额度" min-width="100" />
      <el-table-column prop="betTime" label="下注时间" min-width="100" />
      <el-table-column prop="betAmount" label="投注额度" min-width="100" />
      <el-table-column
        prop="validBetAmount"
        label="有效投注额度"
        min-width="130"
      />
      <el-table-column prop="playType" label="玩法类型" min-width="100" />
      <el-table-column prop="tableCode" label="桌台号" min-width="100" />
      <el-table-column prop="recalcuTime" label="派彩时间" min-width="100" />
      <el-table-column prop="beforeCredit" label="余额" min-width="100" />
      <el-table-column prop="remark" label="注示" min-width="100" />
      <el-table-column prop="seat" label="座位号" min-width="100" />
    </el-table>
...
<script>
import { recordKy } from "@/api/platformGameRecord/agRealPerson";
...
export default {
  name: "platformGameRecordAgRealPersonIndex",
  components: {
    UserDrawer,
  },
  data() {
