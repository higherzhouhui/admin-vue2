    ...
    <el-table v-cloading="listLoading" :data="list" size="mini">
      ...
      <el-table-column label="用户ID" min-width="120">
        <template slot-scope="{ row }">
          <user-drawer :uid="row.userId" />
        </template>
      </el-table-column>
      <el-table-column prop="loginId" label="用户登录ID" min-width="100" />
      <el-table-column prop="sn" label="厅代码" min-width="100" />
      ...
      <el-table-column prop="betId" label="注单号" min-width="160" />
      <el-table-column prop="fireCount" label="发炮数量" min-width="100" />
      <el-table-column prop="betAmount" label="下注额" width="170" />
      <el-table-column
        prop="validAmount"
        label="有效投注"
        width="100"
        sortable
      />
      <el-table-column prop="calcAmount" label="结算金额" min-width="100" />
      <el-table-column prop="payout" label="派彩(输赢)" min-width="100" />
      <el-table-column prop="gameBalance" label="下注后余额" min-width="100" />
      <el-table-column
        prop="orderTime"
        label="下注时间(UTC-4)"
        min-width="140"
      />
      <el-table-column prop="jackpot" label="JP中奖" min-width="100" />
      <el-table-column prop="jackpotTypeString" label="奖池类型" min-width="100" />
      <el-table-column prop="extend" label="JP预抽" min-width="100" />
      <el-table-column prop="orderFromString" label="终端类别" min-width="100" />
      <el-table-column prop="startTime2" label="开始时间" min-width="140" />
      <el-table-column prop="endTime2" label="结算时间" min-width="140" />
    </el-table>
...
<script>
import { recordKy } from "@/api/platformGameRecord/bgCatchFish";
      ...
      recordKy(this.listQuery.limit, this.listQuery.page, this.listQuery)
        .then((res) => {
          ...
          this.list = (res.data.list || []).map((item) => {
            ...
            let jackpotTypeString = item.jackpotType
            if (item.jackpotType == 0) {
              jackpotTypeString = '打中宝箱并获得奖金'
            }
            item.jackpotTypeString = jackpotTypeString
