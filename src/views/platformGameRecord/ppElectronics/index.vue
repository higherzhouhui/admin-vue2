<template>
  <div class="app-container">
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini">
      ...
      <el-table-column prop="playerId" label="rsgID" min-width="100" />
      <el-table-column prop="playSessionId" label="订单号" min-width="100" />
      <el-table-column prop="currency" label="币种" min-width="100" />
      <el-table-column prop="gameId" label="游戏id" width="100" />
      <el-table-column prop="betSum" label="投注金额" min-width="170" />
      <el-table-column prop="winSum" label="赢得金额" min-width="170" />
      <el-table-column prop="jackpot" label="累计金额赢得数量" width="140" />
      <el-table-column prop="startData" label="开始时间" width="150">
        <template slot-scope="{row}">
          {{ formatUnixTime(row.startData) }}
        </template>
      </el-table-column>
        ...
        <el-form ref="dataForm" :inline="true" :model="createModel" label-position="top" label-width="100px" size="medium"
          ...
          <div style="
            ...
            <el-form-item label="补偿日期">
              ...
              <el-date-picker v-model="createModel.day" type="datetime" placeholder="请选择补偿日期" value-format="yyyyMMddHH">
              </el-date-picker>
            </el-form-item>
...
<script>
import { recordPp } from "@/api/platformGameRecord/ppElectronics";
...
export default {
  name: "platformGameRecordPpElectronicsIndex",
  components: {
    UserDrawer,
  },
  data() {
  ...
  methods: {
    ...
    fetchData() {
      this.listLoading = true;
      this.familyList = [];
      recordPp(this.listQuery.limit, this.listQuery.page, this.listQuery)
        .then((res) => {
