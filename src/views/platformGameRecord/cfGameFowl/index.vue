<template>
  <div class="app-container">
    ...
    <el-table v-cloading="listLoading" :data="list" size="mini">
      ...
      <el-table-column prop="slug" label="投注单号" min-width="100" />
      <el-table-column prop="arenaFightNo" label="赛事代号" min-width="100" />
      <el-table-column prop="roundId" label="RoundId" min-width="100" />
      <el-table-column prop="fightNo" label="FightNo" width="100" />
      <el-table-column prop="side" label="下注哪一边" min-width="110">
      </el-table-column>
      <el-table-column prop="account" label="会员账号名" min-width="100" />
      ...
      <el-table-column prop="odd" label="赔率" width="100" />
      <el-table-column prop="betAmount" label="下注金额" min-width="100" />
      <el-table-column prop="netIncome" label="净收入" min-width="100" />
      <el-table-column prop="betReturn" label="返回金额" min-width="100" />
      <el-table-column prop="validAmount" label="有效投注" min-width="100" />
      ...
      <el-table-column prop="isSettledString" label="是否已结算" min-width="120" />
      <el-table-column prop="betAt" label="投注时间" width="100" />
      <el-table-column prop="settledAt" label="结算时间" width="100">
      </el-table-column>
...
<script>
import { recordKy } from "@/api/platformGameRecord/cfGameFowl";
...
export default {
  name: 'PlatformGameRecordCfGameFowlIndex',
  components: {
    UserDrawer,
  },
  data() {
    return {
      ...
      listQuery: {
        ...
        type: null,
        pickerValue: [],
        // pickerValue2: [NOW_TIME_START, NOW_TIME_END],
      },
      newestTime: '',
      ...
      recordKy(this.listQuery.limit, this.listQuery.page, this.listQuery)
        .then((res) => {
          ...
          this.list = (res.data.list || []).map((item) => {
            ...
            // );
            let isSettledString = '是'
            if (!item.isSettled) {
              isSettledString = '否'
            }
            item.isSettledString = isSettledString
