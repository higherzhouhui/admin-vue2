<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="amount" label="取款金额" min-width="95" sortable />
      <el-table-column
        prop="beforeAmount"
        label="申请前账户余额"
        min-width="135"
        sortable
      />
      <el-table-column label="取款信息" min-width="320">
        <template slot-scope="{ row }">
          {{ `${row.bankName}${row.cardNo}${row.trueName}` }}
        </template>
      </el-table-column>

      <el-table-column prop="bankAddress" label="支行地址" min-width="180" />
...
export default {
  name: "AgentManagementWithdrawMoney",
  data() {
    return {
      list: [],
      todayList: null,
